...
namespace imc
{
	...
	public:
		...
		void FordwardPropagate()
		{
			this->forwardPropagate();
		}
		void Backpropagate(double *target, int errorFunction)
		{
		...
		double GetError(double *target, int method)
		{
			this->obtainError(target, method);
		}
		double ShowDeltas()
		{
			int contNeuron = 1;
			for (int i = 1; i < nOfLayers; i++)
			{
				...
				{
					std::cout << "Delta " << contNeuron << ": " << layers[i].neurons[j].delta << std::endl;
					contNeuron++;
				}
			}
		}
